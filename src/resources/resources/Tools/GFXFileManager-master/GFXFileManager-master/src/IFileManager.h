...
typedef void (__cdecl *foreach_callback_t)(CALLBACK_STATE, search_result_t *, void*);
...
public:
	...
	virtual int Open(const char *filename, int access, int length) = 0; //
	...
	// - access: 0 for open-existing, 0x80000000 for open and share_read, 0x40000000 for create_always
	// - unknown: not used for original CPFileManager
	virtual int Open(CJArchiveFm* fm, const char *filename, int access, int length) = 0;

	virtual int Function_12(void) = 0; //return -1
	virtual int Function_13(void) = 0; //return 0

	virtual int Create(const char* filename, int length) = 0; //
	virtual int Create(CJArchiveFm * fm, const char * filename, int length) = 0; //
	...
	virtual __int64* GetDirectoryPosition(__int64* pPosition) = 0;
	virtual bool SetDirectoryPosition(__int64 position) = 0;
	...
	virtual search_handle_t* FindFirstFile(search_handle_t* pSearchHandle, const char* pSearchPattern, search_result_t* pSearchResult) = 0;
	...
	virtual bool FindNextFile(search_handle_t* pSearchHandle, search_result_t* pSearchResult) = 0;

	// Free the search result
	// Parameter:
	// - search: result structure representing a handle for the search
	virtual bool FindClose(search_handle_t* pSearchHandle) = 0;
