...
namespace SecurityAPI
{
    public class Security
    {
        ...
        public uint CountSeed => m_seed_count;
        public uint CRCSeed => m_crc_seed;
        public ulong InitialBlowfish => m_initial_blowfish_key;

        bool m_passive_mode;
        public bool IsPassive { get { return m_passive_mode; } set { m_passive_mode = value; } }
        ...
        byte GenerateCheckByte(byte[] stream, int offset, int length)
        {
            ...
            uint moddedseed = (m_crc_seed & 0xFF) << 8;
        ...
        {
            GenerateSecurity(flags, NextUInt32(), NextUInt32(), NextUInt64());
        }

        public void GenerateSecurity(SecurityFlags flags, uint seedCount, uint seedCRC, ulong initialBlowfishKey)
        {
            ...
            {
                m_initial_blowfish_key = initialBlowfishKey;
         ...
         public void ResyncHandshake(Packet packet)
         {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (!m_passive_mode)
                    {
                        ...
                        // Insert after 0x9000
                        m_outgoing_packets.Add(response2);
                    }
        ...
        public void ClearOutgoingPackets()
        {
            lock (m_class_lock)
            {
                        ...
                        if (m_client_security)
                        {
                            if (m_security_flags.security_bytes == 1)
                            {
                                ...
                                if (packet_opcode != 0x5000 && packet_opcode != 0x9000)
                                {
                                    ...
                                    {
                                        throw (new Exception($"[SecurityAPI::Recv] CRC byte mismatch. Opcode: {packet_opcode:X4} | Expected: {expected_crc:X2} | Got: {packet_security_crc:X2}"));
                                    }
                                }
