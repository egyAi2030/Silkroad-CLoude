...
namespace SRO.PK2API
{
    public class Pk2Stream : IDisposable
    {
        ...
        public Dictionary<string, Pk2File> Files => mFiles;
        #endregion
        ...
        public Pk2Stream(string path, string key, bool isReadOnly = false)
        {
            ...
            // Check file setup
            mFileStream = new FileStream(path, isReadOnly ? FileMode.Open : FileMode.OpenOrCreate, isReadOnly ? FileAccess.Read : FileAccess.ReadWrite, FileShare.ReadWrite);
            if (!fileExists)
                CreateBaseStream();
